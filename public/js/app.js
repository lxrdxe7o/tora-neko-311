...
async function fetchFlights() {
    return apiRequest('flights.php');
}

async function fetchSeats(flightId) {
    return apiRequest(`seats.php?flight_id=${flightId}`);
}

async function createBooking(seatId, passengerName, passportNumber) {
    return apiRequest('book.php', {
        method: 'POST',
        body: JSON.stringify({
            seat_id: seatId,
...
function populateFlightSelect(flights) {
    elements.flightSelect.innerHTML = '<option value="">-- Select a flight --</option>';
    
    flights.forEach(flight => {
        const option = document.createElement('option');
        option.value = flight.id;
        option.textContent = `${flight.flight_number} | ${flight.origin} → ${flight.destination} | ${formatDate(flight.departure_time)} | ${formatPrice(flight.price)}`;
...
function renderSeatMap(seatData) {
    ...
    headerRow.innerHTML = `
        ...
        <div class="seat-group">
            ${legend.columns.slice(0, 3).map(col => `<div class="seat" style="background:transparent;border:none;cursor:default;color:var(--text-muted)">${col}</div>`).join('')}
        </div>
        <div class="aisle"></div>
        <div class="seat-group">
            ${legend.columns.slice(3).map(col => `<div class="seat" style="background:transparent;border:none;cursor:default;color:var(--text-muted)">${col}</div>`).join('')}
        </div>
        <div class="row-label"></div>
    `;
    ...
    seat_map.forEach(row => {
        ...
        row.seats.slice(0, 3).forEach(seat => {
            leftGroup.appendChild(createSeatElement(seat, row.class));
        });
        ...
        row.seats.slice(3).forEach(seat => {
            rightGroup.appendChild(createSeatElement(seat, row.class));
        });
...
function createSeatElement(seat, seatClass) {
    ...
    if (seat.is_booked) {
        ...
        seatDiv.addEventListener('click', () => selectSeat(seat, seatClass));
    }
...
function selectSeat(seat, seatClass) {
    // Deselect previous
    if (state.selectedSeat) {
        const prevSeat = document.querySelector(`.seat[data-seat-id="${state.selectedSeat.id}"]`);
        if (prevSeat) {
    ...
    // Select new
    state.selectedSeat = { ...seat, class: seatClass };
            ...
            <div class="result-section">
                ...
                <div class="result-grid">
                    ...
                    <div class="result-item">
                        ...
                        <div class="value">${booking.flight.origin} → ${booking.flight.destination}</div>
                    </div>
                    <div class="result-item">
                        <div class="label">Departure</div>
                        <div class="value">${formatDate(booking.flight.departure)}</div>
                    </div>
                    <div class="result-item">
                        <div class="label">Seat</div>
                        <div class="value">${booking.seat.label} (${booking.seat.class})</div>
                    </div>
                </div>
            </div>
            ...
            <div class="result-section">
                ...
                <div class="result-grid">
                    <div class="result-item">
                        ...
                        <div class="value">${quantum_security.signature.algorithm}</div>
                    </div>
                    <div class="result-item">
                        <div class="label">Encryption Algorithm</div>
                        <div class="value">${quantum_security.encryption.algorithm}</div>
                    </div>
                </div>
                <div class="result-item" style="margin-top: 1rem;">
                    <div class="label">Digital Signature Preview</div>
                    <div class="signature-preview">${quantum_security.signature.preview}</div>
                </div>
            </div>
        </div>
    `;
...
function renderVerifyResult(result) {
    ...
    elements.verifyResult.className = `verify-result ${isValid ? 'success' : 'failure'}`;
        ...
        elements.verifyResult.innerHTML = `
            ...
            <div class="result-grid" style="margin-top: 1rem;">
                ...
                <div class="result-item">
                    ...
                    <div class="value">${result.security.signature_algorithm}</div>
                </div>
            </div>
            ${result.security.mock_mode ? '<p style="margin-top:1rem;color:var(--warning);font-size:0.75rem;">&#9888; Verified using mock mode (HMAC-SHA512)</p>' : ''}
        `;
    } else {
        elements.verifyResult.innerHTML = `
...
async function handleFlightChange(e) {
    ...
    if (!flightId) {
        ...
        elements.seatMap.innerHTML = '<div class="seat-placeholder">Select a flight to view seat map</div>';
...
async function handleBookingSubmit(e) {
    ...
    try {
        showLoading();
        const result = await createBooking(state.selectedSeat.id, passengerName, passportNumber);
...
async function init() {
    ...
    try {
        ...
        elements.flightSelect.innerHTML = '<option value="">Failed to load flights</option>';
